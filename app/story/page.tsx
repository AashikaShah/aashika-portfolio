...
function CursorTilt({ children }: { children: React.ReactNode }) {
  ...
  return (
    <div
      ...
      style={{
        transform: `perspective(900px) rotateY(${tilt.x}deg) rotateX(${tilt.y}deg) translateZ(${isTilted ? 6 : 0}px)`,
...
function ParallaxImage({
  ...
  const yRange =
    direction === "up" ? [30, -30] : direction === "down" ? [-30, 30] : [0, 0];

  const y = useTransform(scrollYProgress, [sectionStart, sectionEnd], yRange);
...
function Scene({
  ...
  const sectionSlice = (1 - HERO_SLICE) / total;
  const start = HERO_SLICE + index * sectionSlice;
            ...
            {paragraphs.map((p, i) => (
              <p
                ...
                style={{
                  ...
                  lineHeight: 1.95,
                  color: i === 0 ? "rgba(26,26,26,0.88)" : "rgba(26,26,26,0.68)",
                  margin: 0,
                }}
              >
                {p}
...
function HeroScene({ progress }: { progress: number }) {
  const opacity = Math.max(0, 1 - progress / HERO_SLICE);
  const isInteractive = opacity > 0.5;
...
function ProgressBar({
  ...
  const sectionSlice = (1 - HERO_SLICE) / sections.length;
  const rawIndex = Math.floor((progress - HERO_SLICE) / sectionSlice);
  const activeIndex = Math.max(0, Math.min(sections.length - 1, rawIndex));
      ...
      {sections.map((s, i) => (
        <div
          ...
          style={{
            ...
            borderRadius: 999,
            backgroundColor:
              i === activeIndex ? s.accent : "rgba(26,26,26,0.14)",
...
/* ── Hook to subscribe to a motion value as React state ───── */
      ...
      >
        ...
        <div
          style={{
            ...
            opacity: Math.max(0, 1 - progress / HERO_SLICE),
            transition: "opacity 0.1s linear",
            zIndex: 0,
            pointerEvents: "none",
          }}
        ...
        {/* Section backgrounds */}
        {SECTIONS.map((s, i) => {
          const start = HERO_SLICE + i * sectionSlice;
          const end = start + sectionSlice;
          ...
          if (i < SECTIONS.length - 1 && progress >= end) {
            bgOpacity = Math.max(
              0,
              1 - (progress - end) / (sectionSlice * 0.2)
            );
          }
